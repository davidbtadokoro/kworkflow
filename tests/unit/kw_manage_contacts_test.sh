...
function test_create_group()
{
  ...
  # invalid cases
  output=$(create_group "$TEST_GROUP_NAME" 2> /dev/null)
  ret="$?"
  expected='This group already exists'
...
function test_remove_group()
{
  ...
  # invalid cases
  output=$(remove_group "nonexistent_group'" 2> /dev/null)
  ret="$?"
...
function test_rename_group()
{
  ...
  # invalid values
  output=$(rename_group 'old_group_name' "nonexistent_group'" 2> /dev/null)
  ret="$?"
  expected=$'Error while removing group from the database with command:\nsqlite3 -init '
...
function test_add_contacts()
{
  ...
  output=$(add_contacts '_contacts_arr' 2> /dev/null)
  ret="$?"
...
function test_add_contact_group()
{
  ...
  output=$(add_contact_group _contacts_arr "$group_id" 2> /dev/null)
  ret="$?"
...
function test_remove_contact()
{
  ...
  # invalid values
  output=$(remove_contact "abc1'" 2> /dev/null)
  ret="$?"
...
function test_manage_contacts_parser()
{
  ...
  expected='fake_group'
  assert_equals_helper 'Set group-rename' "$LINENO" "${options_values['GROUP']}" "$expected"
  expected='new_group'
  ...
  expected='ctt1 <email1>, ctt2 <email2>'
  assert_equals_helper 'Set group add' "$LINENO" "${options_values['GROUP_ADD']}" "$expected"
  expected='group'
  assert_equals_helper 'Set group add' "$LINENO" "${options_values['GROUP']}" "$expected"
