...
function test_kernel_uninstall_regex_one_kernel()
{
  ...
  local -a cmd_last_part=(
    "Can't find ${TARGET_PATH}//$mkinitcpio_d_path_1"
    "Can't find ${TARGET_PATH}//var/lib/initramfs-tools/$kernel_name_1"
    "Can't find ${TARGET_PATH}//lib/modules/$kernel_name_1"
    "sudo sed -i '/$kernel_name_1/d' '$INSTALLED_KERNELS_PATH'"
...
function test_kernel_uninstall_regex_two_kernels()
{
  ...
  for cmd in "${cmd_last_part_1[@]}"; do
    cmd_sequence["$index"]="$cmd"
    ((index++))
  done

  local -a kernel_2_removal=(
    "Removing: $kernel_name_2"
  )

  for cmd in "${kernel_2_removal[@]}"; do
    cmd_sequence["$index"]="$cmd"
    ((index++))
  done
  ...
  local -a cmd_last_part_2=(
    "Can't find ${TARGET_PATH}//$mkinitcpio_d_path_2"
    "Can't find ${TARGET_PATH}//var/lib/initramfs-tools/$kernel_name_2"
    "Can't find ${TARGET_PATH}//lib/modules/$kernel_name_2"
    "sudo sed -i '/$kernel_name_2/d' '$INSTALLED_KERNELS_PATH'"
    'run_bootloader_update_mock'
  )

  for cmd in "${cmd_last_part_2[@]}"; do
    cmd_sequence["$index"]="$cmd"
    ((index++))
  done
...
function test_kernel_uninstall_unmanaged()
{
  ...
  echo "menuentry 'Arch Linux, with Linux 5.5.0-rc2-NOTMANAGED'" >> "${TARGET_PATH}/boot/grub/grub.cfg"
...
function test_kernel_force_uninstall_unmanaged()
{
  ...
  echo "menuentry 'Arch Linux, with Linux 5.5.0-rc2-NOTMANAGED'" >> "$grub_cfg_path"
  touch "$boot_path"
