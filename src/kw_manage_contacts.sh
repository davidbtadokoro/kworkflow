...
declare -gr DATABASE_TABLE_GROUP='email_group'
declare -gr DATABASE_TABLE_CONTACT='email_contact'
declare -gr DATABASE_TABLE_CONTACT_GROUP='email_contact_group'
...
function manage_contacts_main()
{
  ...
  if [[ "${options_values['GROUP_CREATE']}" ]]; then
    create_email_group "${options_values['GROUP']}"

    if [[ "$?" -eq 0 ]]; then
      success "New group ${options_values['GROUP']} created successfully!"
    fi
  fi

  if [[ "${options_values['GROUP_REMOVE']}" ]]; then
    remove_email_group "${options_values['GROUP']}"

    if [[ "$?" -eq 0 ]]; then
      success "Group ${options_values['GROUP']} removed successfully!"
    fi
  fi

  if [[ -n "${options_values['GROUP_RENAME']}" ]]; then
    rename_email_group "${options_values['GROUP']}" "${options_values['GROUP_RENAME']}"

    if [[ "$?" -eq 0 ]]; then
      success "Group ${options_values['GROUP']} successfully renamed to ${options_values['GROUP_RENAME']}"
    fi
  fi

  if [[ -n "${options_values['GROUP_ADD']}" ]]; then
    add_email_contacts "${options_values['GROUP_ADD']}" "${options_values['GROUP']}"
  ...
  if [[ -n "${options_values['GROUP_REMOVE_EMAIL']}" ]]; then
    ...
    if [[ "$?" -eq 0 ]]; then
      success 'Contact group removed successfully!'
    fi
  fi
...
function remove_email_contact()
{
  ...
  remove_contact_group "$group_id" "$contact_id"

  if [[ "$?" -ne 0 ]]; then
    complain 'Error while removing contact group'
    return 22 # EINVAL
  fi
...
function manage_contacts_help()
{
  ...
  printf '%s\n' 'kw manage-contacts:' \
    ...
    '  manage-contacts --group-remove-email "[<group_name>]:[<contact_name>]" - remove contact from existent group' \
